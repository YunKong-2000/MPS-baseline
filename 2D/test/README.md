...
10. **test_explicit_force** - 显式力计算模块测试（粘性力、重力、速度和位置更新）
...
#### 1. 测试场景设置
- 生成管道流动测试场景（Poiseuille流动）
- 设置初始速度场（理论Poiseuille分布）
- 构建邻居列表和corrective matrix

#### 2. 速度拉普拉斯算子验证
- 计算初始状态的速度拉普拉斯算子
- 与理论值进行比较（Poiseuille流动的理论拉普拉斯算子）
- 统计误差（平均误差和最大误差）
...
- **explicit_force_test.vtk**：包含以下数据：
  - `initial_velocity` 向量：初始速度
  - `final_velocity` 向量：最终速度
  - `velocity_change` 向量：速度变化
  - `initial_position` 向量：初始位置
  - `position_change` 向量：位置变化
  - `velocity_laplacian` 向量：速度拉普拉斯算子
  - `viscous_acceleration` 向量：粘性力加速度
  - `gravity_acceleration` 向量：重力加速度
  - `y_coordinate` 标量：Y坐标（用于分析）
...
速度拉普拉斯算子：
- `∇²v_x = -8·v_max/h²`（常数，与y无关）
- `∇²v_y = 0`

#### 显式时间积分
使用前向欧拉法：
- `v^{n+1} = v^n + a^{total} · Δt`
- `x^{n+1} = x^n + v^{n+1} · Δt`

其中：
- `a^{total} = a_{viscous} + a_{gravity}`
- `a_{viscous} = ν · ∇²v`
- `a_{gravity} = g`
...
1. **速度拉普拉斯算子验证**：
   - 计算值与理论值比较
   - 对于Poiseuille流动，拉普拉斯算子应该是常数（-8·v_max/h²）
   - 误差应该在数值精度范围内
